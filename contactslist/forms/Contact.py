...
class UserContactContact(forms.ModelForm):
    ...
    phone_type = forms.ChoiceField(label="phone_type", choices=UserContact.phone_choices, required=False)
    
    email = forms.CharField(label="email", max_length=50, required=False)
    email_type = forms.ChoiceField(label="email_type", choices=UserContact.common_choices, required=False)
    
    address = forms.CharField(label="address", widget=forms.Textarea, required=False)
    address_type = forms.ChoiceField(label="address_type", choices=UserContact.common_choices, required=False)
    
    occasion_date = forms.DateField(label="occasion_date", required=False)
    occasion = forms.ChoiceField(label="occasion", choices=UserContact.date_choices, required=False)
    
    relation = forms.ChoiceField(label="relationships", choices=UserContact.relations, required=False)
